...
A session is selected only when its recorded `cwd` resolves to the current repository, unless the user supplies an exact session ID. Agent summarization is opt-in and its result remains `agent-inferred`.
